...
public class AiEnemycrossbow : Health, IDamegable
{
[Header("Enemy")]
...
[Header("Animations")]
    ...
    [SpineAnimation][SerializeField] private string rechargeAnimationName;
...
private enum State { Moving, Run, Attack, Knockback, Recharge, Dead, Hurt, Wait }
private State currentState;

public static AiEnemycrossbow instance;
...
private void Run()
{
    ...
    if (isRun)
    {
        ...
        Vector2 newPosition = (Vector2)transform.position + direction.normalized * fleeDistance;
        transform.position = Vector2.MoveTowards(transform.position, newPosition, chaseSpeed * Time.deltaTime);

        // Cambia la direzione in cui guarda il nemico
...
public void Shoot()
{
    ...
    isAttacking = true;
            if (shotCounter <= 0)
            {
                ...
                newBullet.transform.localScale = Brain.transform.localScale;
            }else
            {
                Recharge();
            }
...
{
    if (currentAnimationName != rechargeAnimationName)
                {    
                    _spineAnimationState.ClearTrack(2);
                    _spineAnimationState.SetAnimation(1, rechargeAnimationName, false);
                    currentAnimationName = rechargeAnimationName;
                    _spineAnimationState.Event += HandleEvent;
...
public void AttackAnm()
{
    ...
    if (currentAnimationName != attackAnimationName)
                {
                    _spineAnimationState.SetAnimation(2, attackAnimationName, true);
